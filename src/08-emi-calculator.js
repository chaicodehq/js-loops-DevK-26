...
export function calculateEMI(principal, monthlyRate, emi) {
  ...
  if (typeof principal !== 'number' || principal <= 0 ||
      ...
      typeof emi !== 'number' || emi <= 0) return invalid;

  if (emi <= principal * monthlyRate) return invalid;

  let remaining = principal;
  let months = 0, totalPaid = 0;
  ...
  return { months, totalPaid: Math.round(totalPaid * 100) / 100, totalInterest: Math.round((totalPaid - principal) * 100) / 100 };
}
